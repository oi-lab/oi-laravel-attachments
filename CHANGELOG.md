...
- **Events**: `FileStored`, `FileAttached`, `FileDetached`, `AttachmentsSynced`, and `AttachmentsReordered` are dispatched for each action so host apps can react without overriding package internals.
...
### Testing
- 56 tests covering the attachment trait, models and relationships, sorting, upload actions, file metadata, audit tracking, events, and the configurable model resolver.
